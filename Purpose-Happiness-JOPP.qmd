...
subtitle: "Reliably Related Constructs Over Time"
author: 
...
gender_tbl <- process_categorical(selected_data, "b_Gender", "Gender")
...
apa_table <- flextable(combined_demographics)

# Apply APA formatting
apa_table <- apa_table %>%
  bold(part = "header") %>%
  add_header_lines(values = paste0("Table 1. Demographic Characteristics (N = ", total_n, ")")) %>%
  bold(part = "header") %>%
  align(align = "left", part = "all") %>%
  align(align = "right", j = 3, part = "body") %>%
  border_remove() %>%
  border(i = 1, border.top = fp_border(color = "black", width = 2), part = "header") %>%
  border(i = 1, border.bottom = fp_border(color = "black", width = 1), part = "header") %>%
  border(i = nrow(combined_demographics), border.bottom = fp_border(color = "black", width = 2), part = "body") %>%
  autofit()
...
# save_as_docx(apa_table, path = "demographic_table_apa.docx")

# If you prefer, you can also print as HTML/kable version
combined_demographics %>%
...
#| 
# print results$comparison with kable
results$comparison %>%
...
results$loadings[, c("group", "param_label", "est_se")] %>%
  kable(format = "html", booktabs = TRUE, caption = "Standardized Factor Loadings (estimate (SE))") %>%
...
#| 
# Display the factor correlations over time
...
# Final Analysis: What is Temporal Relationship between Happiness and Purpose?
...
kable(path_table[, c("Path_Type", "Path", "Coefficient", "Std_Coef", "p_value", "Significance")],
  ...
  #footnote(symbol = c("*p < .05", "**p < .01", "***p < .001"),
  #         symbol_title = "Note.",
  #         footnote_as_chunk = TRUE)
```
...
# Create an APA-style table
kable(formatted_table[, c("Timepoint", "Quantile", "Slope", "t_value", "p_value", "Significance")],
      format = "html",
      booktabs = TRUE,
  ...
  kable_styling(full_width = FALSE, position = "center", latex_options = "hold_position") %>%
  row_spec(0, bold = TRUE) %>%
  pack_rows("Baseline", 1, 5, bold = TRUE) %>%
  pack_rows("Follow-up 1", 6, 10, bold = TRUE) %>%
  pack_rows("Follow-up 2", 11, 15, bold = TRUE) #%>%
  #footnote(symbol = c("*p < .05", "**p < .01", "***p < .001"),
  #         symbol_title = "Note.",
  #         footnote_as_chunk = TRUE)
```
