...
analyze_correlates <- function(data) {
  ...
  correlation_data <- data %>%
    select(any_of(c(trait_vars, outcome_vars)))
...
run_correlate_analysis <- function(data) {
  ...
  if(requireNamespace("corrplot", quietly = TRUE)) {
    ...
    corr_vars <- corr_vars[corr_vars %in% names(data)]
    
    if(length(corr_vars) > 1) {
      ...
      # Limit var_labels to those that correspond to corr_vars
      var_labels <- var_labels[1:length(corr_vars)]
      ...
      tryCatch({
        ...
        message("Error creating correlation plot: ", e$message)
      })
    }
  }
...
# ===============================================================
# SECTION 4: Predictive Analysis for Future Outcomes
# ===============================================================
...
analyze_longitudinal_outcomes <- function(data) {
  ...
  # Filter predictors and 6-month outcomes that are in the data
  predictors <- predictors[predictors %in% names(data)]
  outcomes_6mo <- outcomes_6mo[outcomes_6mo %in% names(data)]
  
  # Correlate baseline predictors with 6-month outcomes
  if(length(predictors) > 0 && length(outcomes_6mo) > 0) {
    pred_6mo_data <- data %>%
    ...
    pred_6mo_results <- data.frame(
      Outcome = outcome_labels[1:length(outcomes_6mo)],
      Purpose_6mo = pred_6mo_cor[outcomes_6mo, "b_bpurp_mean"],
      Happiness_6mo = pred_6mo_cor[outcomes_6mo, "b_shs_mean"]
    )
  } else {
    pred_6mo_cor <- NULL
    pred_6mo_results <- NULL
  }
  
  # Filter 2-year outcomes that are in the data
  outcomes_2yr <- outcomes_2yr[outcomes_2yr %in% names(data)]
  
  # Correlate baseline predictors with 2-year outcomes
  if(length(predictors) > 0 && length(outcomes_2yr) > 0) {
    pred_2yr_data <- data %>%
    ...
    pred_2yr_results <- data.frame(
      Outcome = outcome_labels[1:length(outcomes_2yr)],
      Purpose_2yr = pred_2yr_cor[outcomes_2yr, "b_bpurp_mean"],
      Happiness_2yr = pred_2yr_cor[outcomes_2yr, "b_shs_mean"]
    )
  } else {
    pred_2yr_cor <- NULL
    pred_2yr_results <- NULL
  }
  
  # Combine results if both are available
  if(!is.null(pred_6mo_results) && !is.null(pred_2yr_results)) {
    longitudinal_results <- pred_6mo_results %>%
      left_join(pred_2yr_results, by = "Outcome") %>%
      mutate(across(where(is.numeric), ~ round(., 3)))
  } else if(!is.null(pred_6mo_results)) {
    longitudinal_results <- pred_6mo_results %>%
      mutate(across(where(is.numeric), ~ round(., 3)))
  } else if(!is.null(pred_2yr_results)) {
    longitudinal_results <- pred_2yr_results %>%
      mutate(across(where(is.numeric), ~ round(., 3)))
  } else {
...
run_regression_models <- function(data) {
  ...
  # Filter outcomes that exist in the data
  ...
  for(outcome in outcomes_fu1) {
    # Try to run regression models with error handling
    tryCatch({
      ...
      display_name <- case_when(
        model_name == "swls" ~ "Life Satisfaction",
        model_name == "phq" ~ "Depression",
        model_name == "bmpns_A" ~ "Autonomy Satisfaction",
        TRUE ~ model_name
      )
      ...
      new_row <- data.frame(
        ...
        # Purpose-only model
        Purpose_Only_R2 = summary(m1)$r.squared,
        Purpose_Only_B = coef(m1)["b_bpurp_mean"],
        Purpose_Only_p = summary(m1)$coefficients["b_bpurp_mean", "Pr(>|t|)"],
        
        # Happiness-only model
        Happiness_Only_R2 = summary(m2)$r.squared,
        Happiness_Only_B = coef(m2)["b_shs_mean"],
        Happiness_Only_p = summary(m2)$coefficients["b_shs_mean", "Pr(>|t|)"],
        
        # Full model with both predictors
        Full_Purpose_B = coef(m3)["b_bpurp_mean"],
        Full_Purpose_p = summary(m3)$coefficients["b_bpurp_mean", "Pr(>|t|)"],
        Full_Happiness_B = coef(m3)["b_shs_mean"],
        Full_Happiness_p = summary(m3)$coefficients["b_shs_mean", "Pr(>|t|)"],
        Full_Model_R2 = summary(m3)$r.squared
      )
      
      # Add to summary table
      regression_summary <- rbind(regression_summary, new_row)
    }, error = function(e) {
      message("Error in regression model for ", outcome, ": ", e$message)
    })
  }
...
run_purpose_happiness_analysis <- function(data, output_dir = NULL) {
  ...
  required_vars <- c("b_shs_gh_a", "b_bpurp_1")
  ...
  if(!is.null(output_dir)) {
    ...
    if(requireNamespace("ggplot2", quietly = TRUE)) {
      ...
      if(!is.null(stability_results$stability_plots$shs_plot)) {
        ggsave(file.path(output_dir, "happiness_stability.png"), 
               stability_results$stability_plots$shs_plot,
               width = 8, height = 6)
      }
      
      if(!is.null(stability_results$stability_plots$bpurp_plot)) {
...
- **Stability**: Purpose shows higher annual stability compared to happiness.
- **Correlates**: Purpose is more strongly associated with goal-directed variables, while happiness relates more to affective variables.
...
if(!is.null(results$stability$stability_plots)) {
  par(mfrow = c(1, 2))
  print(results$stability$stability_plots$shs_plot)
  print(results$stability$stability_plots$bpurp_plot)
}
```
...
```{r}
if(!is.null(results$longitudinal$regression_summary) && nrow(results$longitudinal$regression_summary) > 0) {
...
# generate_html_report(results, output_file = "purpose_happiness_report.html")# Purpose and Happiness Longitudinal Analysis
...
extract_loadings <- function(model_fit) {
  # Check if model fit is NULL
  if(is.null(model_fit)) {
    return(NULL)
  }
  
  # Try to get standardized parameters
  tryCatch({
    # Extract standardized parameters
    params <- standardizedSolution(model_fit)
    
    # Filter to get loadings
    loadings <- params %>%
      filter(op == "=~") %>%
      mutate(
        timepoint = group,
    ...
    message("Error extracting loadings: ", e$message)
    return(NULL)
  })
}
...
extract_correlations <- function(model_fit) {
  ...
  tryCatch({
    ...
    # Filter to get correlations between latent variables
    correlations <- params %>%
      filter(op == "~~", 
             lhs != rhs, 
             lhs %in% c("shs", "bpurp"), 
    ...
    # Return NULL if there's an error
    message("Error extracting correlations: ", e$message)
    return(NULL)
  })
}

# Function to create beautiful measurement diagram using qgraph
create_measurement_diagram <- function(model_fit, timepoint = NULL) {
  ...
  params <- tryCatch({
    ...
    message("Error getting standardized parameters: ", e$message)
    return(NULL)
  })
  ...
  tryCatch({
    ...
    message("Error creating diagram: ", e$message)
    return(NULL)
  })
}
