...
# Add the columns for the two measures of interest
purp_hap_cols <- c(1,
                     # purpose baseline
                     398:407, 697, 698, 735, 790, 831:838,
                     # FU1
                     1467:1476, 1771, 1772, 1845, 1846, 1859, 1863,
                     # FU2
                     3001:3004, 3542, 3543,# 3556,
                     # happiness baseline
                     241:245, 673, 730, 1208,
                     # fu1
                     1419:1423, 1751, 1825, 1885,
                     # fu2
                     2954:2957, 3428, 3466#, 3555 
)

cols_to_extract <- c(1, cols_to_extract, purp_hap_cols)  # Include the first column (ID)

# Remove duplicates and sort
cols_to_extract <- sort(unique(cols_to_extract))

# Extract the specified columns from the dataframe
dat_paper <- dat[, cols_to_extract, drop = FALSE]
...
format_table <- function(table_data, 
                         caption = NULL, 
                         col_names = NULL, 
                         align = NULL, 
                         digits = NULL) {
...
add_group_rows <- function(table, group_data) {
  if (knitr::is_html_output()) {
    ...
    for (i in seq_along(group_data$group)) {
      table <- table %>%
        add_header_row(values = group_data$group[i], 
                       colwidths = ncol(table)) %>%
        bold(part = "header")
